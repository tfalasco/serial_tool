...
static bool check_output(FILE *fp, uint8_t* packet_buf, uint8_t packet_len)
{
    bool retval = true;
    ...
    {
        // Get the packet from the packet buff
        uint8_t expected[LINE_BUF_SIZE];
        int expected_size = parse_hex_line(line, expected);
        if (expected_size != packet_len)
        {
            printf("Expected output length %d doesn't match actual length %d\n", expected_size, packet_len);
            retval = false;
        }
        for (int i = 0; i < expected_size; i++)
        {
            if (expected[i] != packet_buf[i])
            {
                printf("Expected output %x doesn't match actual output %x\n", expected[i], packet_buf[i]);
                retval = false;
            }
        }
    }else
...
int main(int argc, char **argv)
{
    ...
    {
        printf("Usage:\n");
        printf("  %s <device> <baud> <test_source_file> <expected_results_file> \n", progname(argv[0]));
        return 0;
    }
    ...
    // open the file
    FILE *fp_source = fopen(argv[3], "r");
    if (fp_source == NULL)
    {
        printf("Failed to open test source file. \n");
        exit(1);
    }
    FILE *fp_expected = fopen(argv[4], "r");
    if (fp_expected == NULL)
    {
    ...
    printf("Opened %s @ %d\n", device, baud);
    printf("Running regression test on board\n");
    ...
    long last_tx_time = time_ms();
    bool packet_received = false;
    bool test_finished = false;
    bool final_results = true;
    bool last_test = false;
    bool last_test_rx_received = false; 
    while (!test_finished)
    {
        fd_set readfds;
        ...
        if (packet_len > 0 && (now - last_rx_time) > PACKET_TIMEOUT_MS)
        {
            ...
            bool results = check_output(fp_expected, packet_buf, packet_len);
        ...
        // Change this line to use hex code from file instead
        long now_tx = time_ms();
        if ((now - last_tx_time) > PACKET_TX_WAIT_MS)
        {
            if (fgets(line, sizeof(line), fp_source))
            {
                int len;
                if (line[0] == '"')
